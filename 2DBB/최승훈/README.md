## Inroduction
자율주행을 위한 2DBB 데이터셋을 처리하는 파이프라인입니다. 
특히, 자동차, 트럭, 버스, 특수 차량, 오토바이, 자전거, 보행자, 교통 표지판, 교통 신호등 등의 객체를 감지하기 위한 
2D 바운딩 박스(bounding box)와 관련된 이미지 및 레이블을 처리합니다. 
이 파이프라인을 통해 이미지는 필요한 전처리를 거친 후, 각 객체의 위치와 분류 정보를 포함한 타겟 정보와 함께 반환됩니다.

## CustomDataset & DataLoader
파일 구조는 다음과 같습니다. 공모전에서 제공한 100000장의 사진과 레이블 중 1/10을 사용했습니다.
